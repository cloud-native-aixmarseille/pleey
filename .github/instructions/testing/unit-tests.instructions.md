...
- Keep Arrange explicit inside each test when practical; avoid `beforeEach` and `afterEach` in colocated spec files because they hide the setup that matters for a failing example.
...
- Do not define inline `create*Fixture` or `create*Mock` helpers inside spec files. Shared setup belongs in `src/test-utils/` so defaults and doubles stay consistent across suites.
- Prefer fixture defaults and override only the fields relevant to the scenario under test.
- Avoid overlapping assertions for the same execution path. Each test should prove one behavior, not re-prove the same failure with multiple async expectation chains.
