...
@Injectable()
export class AdvanceStageUseCase extends AbstractHostPartyRuntimeUseCase {
  constructor(
  ...
  execute(input: HostPartyControlDto): Promise<void> {
    return this.executeTransition(input, async (party) =>
      this.hostPartyLifecyclePolicy.advanceStage(this.toLifecycleState(party), {
        nextStage: await this.hostPartyRuntimeStageReferenceResolver.findNextStage(party),
      }),
    );
  }
}
