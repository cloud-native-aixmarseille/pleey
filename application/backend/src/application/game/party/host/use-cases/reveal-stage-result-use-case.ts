...
@Injectable()
export class RevealStageResultUseCase extends AbstractHostPartyRuntimeUseCase {
  constructor(
  ...
  execute(input: HostPartyControlDto): Promise<void> {
    return this.executeTransition(input, (party) =>
      this.hostPartyLifecyclePolicy.revealStageResult(this.toLifecycleState(party)),
    );
  }
}
