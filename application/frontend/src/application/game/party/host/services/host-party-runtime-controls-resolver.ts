...
export class HostPartyRuntimeControlsResolver implements PartyHostRuntimeControlsPort {
  resolveControls(party: PartyObservation): HostPartyRuntimeControlsState {
    ...
    const currentStageNumber =
      lifecycle?.stagePosition === null || lifecycle?.stagePosition === undefined ? null : lifecycle.stagePosition + 1;
    ...
    const canControlActiveRuntime =
      (party.status === PartyStatus.ACTIVE || party.status === PartyStatus.PAUSED) && hasPlayableLifecycle;
    const hasNextStage =
    ...
    return {
      canAdvanceStage: party.status === PartyStatus.ACTIVE && lifecyclePhase === PartyRuntimePhase.RESULT,
      ...
      canRevealStageResult: party.status === PartyStatus.ACTIVE && lifecyclePhase === PartyRuntimePhase.STAGE,
