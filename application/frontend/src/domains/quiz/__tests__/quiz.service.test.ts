...
import type { CreateQuestionPayload, UpdateQuestionPayload } from '../quiz.service';
...
vi.mock('../../../shared/api/openapiClient', () => ({
  apiClient: {
...
describe('QuizService', () => {
  ...
  describe('getQuizzes', () => {
    it('should fetch all quizzes', async () => {
      ...
      expect(mocks.queryOptionsMock).toHaveBeenCalledWith('get', '/api/quizzes');
      expect(mocks.queryClientFetchQueryMock).toHaveBeenCalledWith(queryOptions);
  ...
  describe('createQuiz', () => {
    it('should create a new quiz and invalidate cache', async () => {
      ...
      mocks.fetchClientPostMock.mockResolvedValueOnce({ data: newQuiz, error: null });

      const result = await quizService.createQuiz(mockToken, 'New Quiz', 'New description', 10);

      expect(mocks.fetchClientPostMock).toHaveBeenCalledWith('/api/quizzes', {
        body: { title: 'New Quiz', description: 'New description', organizationId: 10 },
        headers: {
          Authorization: `Bearer ${mockToken}`,
          'Content-Type': 'application/json',
        },
      });
      expect(mocks.queryClientInvalidateMock).toHaveBeenCalledWith({ queryKey: ['get', '/api/quizzes'] });
  ...
  describe('getQuestions', () => {
    it('should fetch questions for a quiz', async () => {
      ...
      expect(mocks.queryOptionsMock).toHaveBeenCalledWith('get', '/api/quizzes/{quizId}/questions', {
  ...
  describe('addQuestion', () => {
    it('should add a question to a quiz and return created question', async () => {
      ...
      globalFetchMock.mockResolvedValueOnce({ ok: true, json: async () => createdQuestion });
  ...
  describe('updateQuestion', () => {
    it('should update a question and return the payload', async () => {
      ...
      globalFetchMock.mockResolvedValueOnce({ ok: true, json: async () => updatedQuestion });
