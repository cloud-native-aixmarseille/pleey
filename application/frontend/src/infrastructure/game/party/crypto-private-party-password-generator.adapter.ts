...
const PRIVATE_PARTY_PASSWORD_ALPHABET = 'ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789!@#$%*';
  ...
  generatePrivatePartyPassword(): string {
    if (typeof crypto !== 'undefined' && typeof crypto.getRandomValues === 'function') {
      ...
      return Array.from(randomValues)
        .map((value) => PRIVATE_PARTY_PASSWORD_ALPHABET[value % PRIVATE_PARTY_PASSWORD_ALPHABET.length] as string)
        .join('');
    }
