...
describe('GraphqlProjectRepository', () => {
  describe('getProjectsByOrganization()', () => {
    it('returns normalized projects for the workspace organization', async () => {
      ...
      const repository = new GraphqlProjectRepository(client);

      // Act
      const projects = await repository.getProjectsByOrganization(3);
    ...
    it('maps transport failures to translated project error keys', async () => {
      ...
      // Act + Assert
      await expect(repository.getProjectsByOrganization(99)).rejects.toThrow(
  ...
  describe('createProject()', () => {
    ...
    it('maps create failures to translated project error keys', async () => {
      ...
      await expect(
        repository.createProject({ organizationId: 3, name: 'Spring Event', description: null }),
  ...
  describe('updateProject()', () => {
    ...
    it('maps update failures to translated project error keys', async () => {
      ...
      await expect(
        repository.updateProject({ projectId: 22, name: 'Spring Event 2', description: null }),
  ...
  describe('deleteProject()', () => {
    it('deletes a project through the GraphQL mutation', async () => {
      ...
      await expect(
        repository.deleteProject({ projectId: 22, migrationProjectId: 31 }),
    ...
    it('maps delete failures to translated project error keys', async () => {
      ...
      await expect(
        repository.deleteProject({ projectId: 22, migrationProjectId: null }),
