...
export function PlayerPartyStatusBar({ onLeaveParty, pausedText, party, variant }: PlayerPartyStatusBarProps) {
  ...
  return (
    <PlayerLobbyStatusBar
      ...
      compact={isCompact}
      metadataBadges={party.host ? [t('game.party.player.route.currentHost', { username: party.host.username })] : []}
...
function resolveSupportingText(
  ...
  switch (variant) {
    ...
    case 'lobby':
      return party.host ? t('game.party.player.route.waitingForHost') : t('game.party.player.route.hostMissing');
    case 'paused-only':
      return party.status === PartyStatus.PAUSED ? (pausedText ?? t('game.party.player.route.actionPaused')) : null;
  }
}
