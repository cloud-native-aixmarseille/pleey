    ...
    t: (key: string, options?: { returnObjects?: boolean }) => {
      if (options?.returnObjects && key === 'home.howItWorks.steps') {
        return [
          ...
          { title: 'Invite players', description: 'Share the pin with participants.' },
        ];
      }

      if (options?.returnObjects && key === 'home.features.items') {
        return [
          { label: 'Fast setup', description: 'Launch a session in seconds.' },
...
describe('home-content-sections', () => {
  ...
  it('renders the how-it-works steps from translated objects', () => {
    ...
    expect(screen.getByText('Share the pin with participants.')).toBeInTheDocument();
  });
