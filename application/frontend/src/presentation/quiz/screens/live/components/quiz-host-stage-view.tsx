...
export function QuizHostStageView({ stage, resolvedActions, isPaused }: HostStageViewProps) {
  const { t } = usePresentationTranslation();

  return (
    <div
      style={{ ...hostStageContainerStyle, position: 'relative' }}
      data-testid="quiz-host-stage-layout"
    ...
    >
      ...
      {isPaused ? (
        <div style={pauseOverlayStyle}>
          <span style={pauseLabelStyle}>{t('quiz.runtime.paused')}</span>
        </div>
      ) : null}
    </div>
  );
