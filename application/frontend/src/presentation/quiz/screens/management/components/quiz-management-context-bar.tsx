...
interface QuizManagementContextBarProps {
...
export function QuizManagementContextBar({
  quizTitle,
  questionCount,
  createdAt,
}: QuizManagementContextBarProps) {
  ...
  return (
    <div style={barStyle} role="banner" aria-label={t('quiz.management.contextBarLabel')}>
      <div style={titleGroupStyle}>
        <span style={iconStyle}>
          <AppIcon name="quiz" size={18} />
        </span>
        <p style={titleStyle}>{quizTitle}</p>
      </div>

      <div style={metaGroupStyle}>
        <p style={metaStyle}>
          {t('quiz.management.questionSummary', { count: String(questionCount) })}
        </p>
        <p style={metaStyle}>{t('quiz.management.createdAt', { date: formattedDate })}</p>
      </div>
    </div>
  );
}
