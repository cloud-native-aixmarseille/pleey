...
type ArcadeToggleTopSpacing = "none" | "sm" | "md";

type ToggleTone = ArcadeBadgeTone;

const GROUP_BASE_CLASSES = "flex flex-wrap gap-3";

const TOP_SPACING_CLASS_MAP: Record<ArcadeToggleTopSpacing, string> = {
  none: "",
  sm: "mt-2",
  md: "mt-3",
};
...
export interface ArcadeToggleGroupProps<T extends string> {
  ...
  multiSelect?: boolean;
  topSpacing?: ArcadeToggleTopSpacing;
  ariaLabel?: string;
}
  ...
  return (
    <div
      className={composeClasses(
        GROUP_BASE_CLASSES,
        TOP_SPACING_CLASS_MAP[topSpacing]
      )}
