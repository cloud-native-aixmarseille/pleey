...
describe('OrganizationProjectList', () => {
  ...
  it('renders projects, selected state, fallback description, and row actions', async () => {
    ...
    expect(
      within(selectedArticle as HTMLElement).getByText(
        'organization.management.projectsSelectedBadge',
      ),
    ).toBeInTheDocument();
  ...
  it('disables removal when the current user cannot remove projects', () => {
    ...
    expect(
      screen.getByRole('button', { name: 'organization.management.projectsRemoveButton' }),
    ).toBeDisabled();
  });
