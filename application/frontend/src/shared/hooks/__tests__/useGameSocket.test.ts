...
describe('useGameSocket', () => {
  ...
  it('should register socket event listeners on mount', () => {
    ...
    expect(socket.on).toHaveBeenCalledWith('player-joined', expect.any(Function));
    expect(socket.on).toHaveBeenCalledWith('game-started', expect.any(Function));
    expect(socket.on).toHaveBeenCalledWith('answer-result', expect.any(Function));
    expect(socket.on).toHaveBeenCalledWith('next-question', expect.any(Function));
